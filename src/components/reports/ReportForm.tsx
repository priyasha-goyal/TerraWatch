...
import { aiServiceShell } from '../../services/ai';
import type { Report } from '../../types';
import { Camera, Brain, MapPin, Loader2, Sparkles, CheckCircle2 } from 'lucide-react';
...
export const ReportForm: React.FC<ReportFormProps> = ({ onSubmit, isSubmitting = false }) => {
  ...
  // Image Upload & AI mock variables
  ...
  const handleMockImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Call service shell mock
    aiServiceShell.detectWaste(file).then((result) => {
      // Set AI recommendation state
      setAiResult({
        wasteType: result.wasteType as keyof typeof WASTE_TYPES,
        severity: result.severity,
        confidence: result.confidence,
        notes: result.ecologicalThreatNotes,
  ...
  const applyAiRecommendations = () => {
    if (!aiResult) return;
    setWasteType(aiResult.wasteType);
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          ...
          // Fallback to random coordinate offset
          ...
          <div className="relative h-60 w-full rounded-xl border border-dashed border-forest-800 bg-slate-900/50 flex flex-col items-center justify-center overflow-hidden transition-all hover:border-emerald-500/40">
            {imageUrl ? (
              <>
                ...
                <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity">
                  <label className="cursor-pointer bg-slate-900/90 border border-slate-700 rounded-lg px-4 py-2 text-xs font-semibold text-white hover:bg-slate-800">
                    Replace Photograph
                    <input type="file" accept="image/*" className="hidden" onChange={handleMockImageUpload} />
                  </label>
                </div>
              </>
            ) : (
              <label className="cursor-pointer flex flex-col items-center justify-center p-6 text-center h-full w-full">
                ...
                <span className="text-xs text-slate-500 mt-1">Upload dump incident context image for AI classification analysis</span>
                <input type="file" accept="image/*" className="hidden" onChange={handleMockImageUpload} />
              </label>
            )}

            {isScanning && (
              <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-sm flex flex-col items-center justify-center text-emerald-400 gap-3">
                ...
                <div className="text-center">
                  ...
                  <p className="text-xs text-slate-500 mt-1">Identifying waste type, volume, and biological hazards</p>
                </div>
              </div>
            )}
          </div>

          {/* AI classification panel suggestion feedback */}
          {aiResult && !isScanning && (
            <div className="rounded-xl border border-emerald-500/20 bg-emerald-950/10 p-5 space-y-3 animate-fade-in-up">
              <div className="flex items-center justify-between border-b border-emerald-950/40 pb-2">
                <div className="flex items-center gap-2 text-emerald-400">
                  <Brain className="h-5 w-5" />
                  <span className="text-xs font-bold uppercase tracking-wider">AI Classification Recommendations</span>
                </div>
                <div className="flex items-center gap-1 text-[11px] font-semibold text-emerald-400 bg-emerald-500/10 rounded-full px-2 py-0.5 border border-emerald-500/20">
                  <Sparkles className="h-3 w-3" />
                  <span>{(aiResult.confidence * 100).toFixed(0)}% Match</span>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-4 text-xs">
                <div>
                  <p className="text-slate-500">Suggested Type</p>
                  <p className="font-semibold text-white mt-0.5">{WASTE_TYPE_DETAILS[aiResult.wasteType]?.label}</p>
                </div>
                <div>
                  <p className="text-slate-500">Estimated Severity</p>
                  <p className="font-semibold text-rose-400 mt-0.5">{aiResult.severity}</p>
                </div>
              </div>
              ...
              <div className="text-xs">
                ...
                <div className="flex flex-wrap gap-1.5 mt-1">
                  {aiResult.detected.map((item, idx) => (
                    <span key={idx} className="bg-slate-900 border border-forest-900/40 rounded px-2 py-0.5 text-[10px] text-slate-300">
                      {item}
                    </span>
                  ))}
                </div>
              ...
              <div className="text-[11px] leading-relaxed text-slate-400 border-t border-emerald-950/20 pt-2">
                <strong>Ecosystem Threat Notice:</strong> {aiResult.notes}
              </div>

              <button
                type="button"
                onClick={applyAiRecommendations}
                className="w-full flex items-center justify-center gap-1.5 rounded-lg bg-emerald-500 hover:bg-emerald-600 px-4 py-2 text-xs font-bold text-white transition-all shadow-md shadow-emerald-500/10"
              >
                <CheckCircle2 className="h-4 w-4" />
                <span>Apply AI Classifications</span>
              </button>
            </div>
          )}
        </div>
